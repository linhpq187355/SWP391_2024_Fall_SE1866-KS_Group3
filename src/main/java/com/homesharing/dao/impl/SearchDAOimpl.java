...
public class SearchDAOimpl implements SearchDAO {
    ...
    @Override
    public List<Home> searchByBedroomCount(int minBedroom, int maxBedroom) {
        String sql = "SELECT h.id, h.name, h.address, h.longitude, h.latitude, h.orientation, h.area, " +
                "h.leaseDuration, h.moveInDate, h.numOfBedroom, h.numOfBath, h.createdDate, " +
                "h.modifiedDate, h.homeDescription, h.tenantDescription, h.wardsId, h.homeTypeId, " +
                "h.createdBy " +
                "FROM Homes h " +
                "WHERE h.numOfBedroom BETWEEN ? AND ?";

        List<Home> homes = new ArrayList<>();
        try (Connection connection = DBContext.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(sql)) {

            preparedStatement.setInt(1, minBedroom);
            preparedStatement.setInt(2, maxBedroom);
            ...
            throw new RuntimeException("Error searching homes by bedroom count: " + e.getMessage(), e);
        }
        return homes;
    }

    @Override
    public List<Home> searchByBathroomCount(int minBathroom, int maxBathroom) {
        String sql = "SELECT h.id, h.name, h.address, h.longitude, h.latitude, h.orientation, h.area, " +
                "h.leaseDuration, h.moveInDate, h.numOfBedroom, h.numOfBath, h.createdDate, " +
                "h.modifiedDate, h.homeDescription, h.tenantDescription, h.wardsId, h.homeTypeId, " +
                "h.createdBy " +
                "FROM Homes h " +
                "WHERE h.numOfBath BETWEEN ? AND ?";

        List<Home> homes = new ArrayList<>();
        try (Connection connection = DBContext.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(sql)) {

            preparedStatement.setInt(1, minBathroom);
            preparedStatement.setInt(2, maxBathroom);
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    Home home = new Home();
                    home.setId(resultSet.getInt("id"));
            ...
            throw new RuntimeException("Error searching homes by bathroom count: " + e.getMessage(), e);
        }
        return homes;
    }

    @Override
    public List<Home> searchByArea(int minArea, int maxArea) {
        String sql = "SELECT h.id, h.name, h.address, h.longitude, h.latitude, h.orientation, h.area, " +
                "h.leaseDuration, h.moveInDate, h.numOfBedroom, h.numOfBath, h.createdDate, " +
                "h.modifiedDate, h.homeDescription, h.tenantDescription, h.wardsId, h.homeTypeId, " +
                "h.createdBy " +
                "FROM Homes h " +
                "WHERE h.area BETWEEN ? AND ?";

        List<Home> homes = new ArrayList<>();
        try (Connection connection = DBContext.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(sql)) {

            preparedStatement.setInt(1, minArea);
            preparedStatement.setInt(2, maxArea);
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    Home home = new Home();
                    home.setId(resultSet.getInt("id"));
            ...
            throw new RuntimeException("Error searching homes by area: " + e.getMessage(), e);
        }
        return homes;
    }
